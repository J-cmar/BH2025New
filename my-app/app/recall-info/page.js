...
export default function RecallInformationPage() {
	...
	const handleResultClick = async (name) => {
		...
		// Calculate date range for report_date
		const now = new Date();
		...
		const url = `https://api.fda.gov/drug/enforcement.json?search=report_date:[${startDate}+TO+${endDate}]+AND+product_description:${name}&limit=10`;
